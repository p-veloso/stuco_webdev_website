...
void setup() {  
  img = loadImage("..\\profile\\profile_low_low.jpg");
  size (2048, 1024);
  ...
  // ------ load shapes ------
  // replace this location with a folder on your machine or use selectFolder()  print(dataPath(""));
  File dir = new File(dataPath("pixels"));  
  if (dir.isDirectory()) {
...
void buildImage(){
  ...
  for (int gridX = 0; gridX < img.width; gridX++) {
    for (int gridY = 0; gridY < img.height; gridY++) {
      ...
      // greyscale conversion      print(red(c), green(c), blue(c));
...
void keyReleased() {
  ...
  if (key == 's' || key == 'S') saveTransparentCanvas(255, timestamp()+"_##.png");
